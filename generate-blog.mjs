...
const recentHistory = posts.slice(0, 60).map(p => ({
  date: p.date, title: p.title, category: p.category, excerpt: p.excerpt
}));
...
Today is ${today} (IST). Select ONE high-value topic for today's TrustRoute Journal article.

MISSION
Find a genuinely useful topic that an Indian office commuter, woman commuter, HR leader, facilities leader, employer, mobility professional, or city resident would care about. The article must teach the reader something substantial and then naturally explain why verified, coordinated carpooling is increasingly relevant. TrustRoute should be positioned as a practical example of that solution, never as a forced advertisement.

TREND DISCOVERY
Use fresh public web research. Look for developments from the last 7 days when a current-news angle exists, while also allowing evergreen topics when they have stronger reader value. Consider:
${editorialPillars.map(x => `- ${x}`).join("\n")}

Examples of angles (do NOT simply copy these): traffic/congestion stories, major bus or public-transport crowding, parking pressure, commuting costs, women commuting safety, corporate transport strategy, empty-seat inefficiency, carpooling benefits, shared mobility, EV/shared-ride interaction, first/last-mile gaps, road-safety developments, new mobility policy.

QUALITY RULES
- Prefer primary/government/research/credible news sources.
...
RECENT TRUSTROUTE ARTICLES (avoid duplication):
...
const topic = await callOpenAI({
  ...
  schema: {
    ...
    properties: {
      title: { type: "string" }, angle: { type: "string" }, category: { type: "string" },
      reason: { type: "string" }, source_urls: { type: "array", items: { type: "string" } }
    },
    required: ["title", "angle", "category", "reason", "source_urls"]
  }
});
...
Candidate sources from research: ${JSON.stringify(topic.source_urls)}

Write a genuinely useful, original article for Indian readers. Research the topic again on the public web before writing. Use fresh sources and verify every factual claim.

ARTICLE GOAL
The reader should finish thinking: "I understand this commuting problem better, and I understand why a verified, coordinated carpool network is a practical part of the answer."

TRUSTROUTE POSITIONING
Mention TrustRoute naturally, usually after the problem and evidence have been explained. Explain relevant product ideas accurately when applicable: Aadhaar/eKYC verification, employer trust signals, driver/vehicle checks, fixed fares, group chat, live location, reliability information, women-only ride matching, and direct rider-to-owner payment. Do not claim TrustRoute guarantees safety, eliminates traffic, is the safest service, has partnerships, users, cities, statistics or outcomes unless explicitly supported by the provided site context or a source.

STYLE
- 900-1400 words.
- Clear Indian English.
- Strong opening, useful subheadings, practical examples.
- No generic AI openings such as "In today's fast-paced world".
- No keyword stuffing.
- No fake quotes.
- No fabricated first-hand experiences.
- No sensationalism.
- Do not simply rewrite one news article; synthesize multiple sources and add analysis.
- If the topic is news-driven, clearly distinguish confirmed facts from interpretation.
- Use HTML only in body_html.
- Include inline source links for important factual claims using <a href="URL" rel="nofollow noopener" target="_blank">Source</a>.
- Finish with a useful takeaway and a restrained TrustRoute connection, not a hard sales pitch.
...
Review the proposed article below. Research the public web again and independently verify the important factual claims. Reject or repair unsupported claims, outdated numbers, weak sources, duplicated ideas, generic AI phrasing, excessive promotion, and any statement that overpromises TrustRoute's capabilities.
...
If any key claim cannot be verified, rewrite it conservatively or remove it. If the topic itself has become unsuitable or unverifiable, set publish=false.
...
const edited = await callOpenAI({
  ...
  schema: {
    type: "object", additionalProperties: false,
    properties: {
      title: { type: "string" }, category: { type: "string" }, excerpt: { type: "string" }, body_html: { type: "string" },
      ...
      quality_score: { type: "integer", minimum: 0, maximum: 100 }, publish: { type: "boolean" }, editor_notes: { type: "string" }
    },
    required: ["title", "category", "excerpt", "body_html", "sources", "quality_score", "publish", "editor_notes"]
  }
});

if (!edited.publish || edited.quality_score < minQualityScore) {
  throw new Error(`Editorial gate blocked publication. score=${edited.quality_score}; publish=${edited.publish}; notes=${edited.editor_notes}`);
}
...
<meta property="og:title" content="${escapeHtml(post.title)} — TrustRoute">
<meta property="og:description" content="${escapeHtml(post.excerpt)}">
<meta property="og:type" content="article">
<meta property="og:url" content="https://trustroute.app/blog/${slug}.html">
<script type="application/ld+json">${JSON.stringify({
  "@context": "https://schema.org", "@type": "Article", headline: post.title,
  description: post.excerpt, datePublished: today, dateModified: today,
  mainEntityOfPage: `https://trustroute.app/blog/${slug}.html`,
  publisher: { "@type": "Organization", name: "TrustRoute", url: "https://trustroute.app/" }
})}</script>
</head>
<body>
...
posts.unshift({ date: today, title: post.title, slug, category: post.category, excerpt: post.excerpt, sources: post.sources });
await fs.writeFile(postsPath, JSON.stringify(posts.slice(0, 60), null, 2), "utf8");
await updateSitemap(posts);

console.log(JSON.stringify({ published: true, title: post.title, slug, quality_score: edited.quality_score, date: today }));

async function callOpenAI({ model, apiKey, input, tools, schemaName, schema }) {
...
function slugify(s) {
  return s.toLowerCase().normalize("NFKD").replace(/[^\w\s-]/g, "").trim().replace(/\s+/g, "-").replace(/-+/g, "-").slice(0, 90);
}
